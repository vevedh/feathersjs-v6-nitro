...
Write-Host "[Patch 010] Node.js $nodeVersionText"

$pnpmCommand = Get-Command pnpm -ErrorAction SilentlyContinue
if ($null -ne $pnpmCommand) {
  $pnpmVersion = (& pnpm --version).Trim()
  if ($pnpmVersion -ne '9.15.9') {
    Write-Host "[Patch 010] pnpm $pnpmVersion detected; pnpm 9.15.9 is required."
    $pnpmCommand = $null
  }
}
...
if ($null -eq $pnpmCommand) {
  ...
  Write-Host '[Patch 010] Preparing pnpm 9.15.9 with Corepack'
...
Write-Host '[Patch 010] Deterministic install with frozen lockfile'
Invoke-Pnpm @('install', '--frozen-lockfile')

Write-Host '[Patch 010] Checking package and toolchain contracts'
Invoke-Pnpm @('check:contract')
Invoke-Pnpm @('check:toolchain')

Write-Host '[Patch 010] Running complete release gate'
Invoke-Pnpm @('verify:release')

Write-Host '[Patch 010] Reproducible alpha.8 baseline validated.'
