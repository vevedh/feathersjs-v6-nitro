...
Write-Host "[Patch 013] Node.js $nodeVersionText"

$pnpmCommand = Get-Command pnpm -ErrorAction SilentlyContinue
if ($null -ne $pnpmCommand) {
  $pnpmVersion = (& pnpm --version).Trim()
  if ($pnpmVersion -ne '9.15.9') {
    Write-Host "[Patch 013] pnpm $pnpmVersion detected; pnpm 9.15.9 is required."
    $pnpmCommand = $null
  }
}
...
if ($null -eq $pnpmCommand) {
  ...
  Write-Host '[Patch 013] Preparing pnpm 9.15.9 with Corepack'
...
Write-Host '[Patch 013] Deterministic install with frozen lockfile'
Invoke-Pnpm @('install', '--frozen-lockfile')

Write-Host '[Patch 013] Checking package, toolchain and Trusted Publishing contracts'
Invoke-Pnpm @('check:contract')
Invoke-Pnpm @('check:toolchain')
Invoke-Pnpm @('check:publishing')

Write-Host '[Patch 013] Running complete release gate'
Invoke-Pnpm @('verify:release')

Write-Host '[Patch 013] Confirming that local publish is blocked'
Invoke-Pnpm @('check:local-publish-guard')

Write-Host '[Patch 013] Local baseline validated. Actual OIDC publication must run in GitHub Actions.'
